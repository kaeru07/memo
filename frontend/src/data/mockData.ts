...
export function initMockData() {
  ...
  const scenarios: Scenario[] = [
    ...
    },
    {
      id: 'scenario-02',
      name: 'ブルートフォース攻撃検知',
      description: 'SSH(22番ポート)へのブルートフォース攻撃を実行し、検知アラートへの対応・アカウントロック・ポート閉鎖を体験する中級シナリオ。',
      category: 'exploitation',
      difficulty: 'intermediate',
      objectives: [
        { id: 'obj-02-1', description: 'SSH Brute Force攻撃を3回以上実行する', completed: false },
        { id: 'obj-02-2', description: 'ブルートフォース検知アラートを確認する', completed: false },
        { id: 'obj-02-3', description: 'レート制限またはIPブロックでアカウントをロックする', completed: false },
        { id: 'obj-02-4', description: 'SSHポート22を閉鎖して再攻撃を封鎖する', completed: false },
      ],
      steps: [
        { order: 1, title: 'Brute Force実行', description: '攻撃コンソールで SSH Brute Force を3回以上実行', successCondition: 'ssh_attempt × 3以上' },
        { order: 2, title: 'アラート確認', description: 'brute_force_detected アラートを確認・対応済みにする', successCondition: 'アラートacknowledged' },
        { order: 3, title: 'アカウントロック', description: 'レート制限またはIPブロックを適用', successCondition: 'rate_limit or block_ip applied' },
        { order: 4, title: 'ポート閉鎖', description: 'SSHポート22をblock_portでブロック', successCondition: 'block_port port:22 applied' },
      ],
      allowedAttackTypes: ['ssh_attempt', 'brute_force', 'tcp_connect'],
      allowedDefenseTypes: ['block_ip', 'rate_limit', 'block_port'],
      timeLimit: 1800,
      tags: ['brute-force', 'ssh', 'account-lock', 'firewall'],
      metadata: {},
    },
    {
      id: 'scenario-03',
      name: '異常通信検知',
      description: 'マルウェアによるC2通信をシミュレートし、異常トラフィック検知・アラート対応・ノード隔離を体験する中級シナリオ。',
      category: 'forensics',
      difficulty: 'intermediate',
      objectives: [
        { id: 'obj-03-1', description: '不審なTCP接続（C2通信シミュレーション）を発生させる', completed: false },
        { id: 'obj-03-2', description: '異常トラフィックアラートを確認・対応済みにする', completed: false },
        { id: 'obj-03-3', description: '感染ノードをネットワークから隔離する', completed: false },
        { id: 'obj-03-4', description: '隔離後に再通信を試みて遮断を確認する', completed: false },
      ],
      steps: [
        { order: 1, title: 'C2通信発生', description: '攻撃コンソールでTCP Connectを実行（ポート4444など）', successCondition: 'tcp_connect イベント発生' },
        { order: 2, title: '異常検知', description: 'anomalous_trafficアラートを確認・対応済みにする', successCondition: 'アラートacknowledged' },
        { order: 3, title: 'ノード隔離', description: '防御ページでノード隔離を適用', successCondition: 'isolate_node applied' },
        { order: 4, title: '遮断確認', description: '再度TCP Connectを実行して通信遮断を確認', successCondition: 'tcp_connect × 2以上' },
      ],
      allowedAttackTypes: ['tcp_connect', 'ping'],
      allowedDefenseTypes: ['isolate_node', 'block_ip', 'firewall_rule'],
      timeLimit: 1800,
      tags: ['malware', 'c2', 'network-isolation', 'forensics'],
      metadata: {},
    },
    {
      id: 'scenario-04',
      name: 'ポート異常アクセス',
      description: '不審なポートへのアクセスを検知し、Firewallルール追加・再アクセス確認を通じてポート管理の基礎を学ぶ上級シナリオ。',
      category: 'defense',
      difficulty: 'advanced',
      objectives: [
        { id: 'obj-04-1', description: '不審なポートへのTCPアクセスを発生させる', completed: false },
        { id: 'obj-04-2', description: '異常アクセスアラートが発生することを確認する', completed: false },
        { id: 'obj-04-3', description: 'Firewallルールまたはポート閉鎖でアクセスをブロックする', completed: false },
        { id: 'obj-04-4', description: '再アクセスを試みてブロックを確認する', completed: false },
      ],
      steps: [
        { order: 1, title: '不審アクセス', description: '攻撃コンソールでTCP Connectを実行（ポート8888など）', successCondition: 'tcp_connect イベント発生' },
        { order: 2, title: 'アラート確認', description: 'anomalous_trafficアラートが発生することを確認', successCondition: 'アラート生成' },
        { order: 3, title: 'Firewall設定', description: '防御ページでFirewallルールかポート閉鎖を適用', successCondition: 'firewall_rule or block_port applied' },
        { order: 4, title: '再確認', description: '再度TCP Connectを実行してブロックを確認', successCondition: 'tcp_connect × 2以上' },
      ],
      allowedAttackTypes: ['tcp_connect', 'ping', 'port_scan'],
      allowedDefenseTypes: ['firewall_rule', 'block_port', 'rate_limit'],
      timeLimit: 1800,
      tags: ['port-security', 'firewall', 'access-control'],
      metadata: {},
    },
  ]
